...
class controlViewFrame(Frame):
    ...
    def createWidget(self):
        ...
        # 配置区
        self.setFrame = tk.LabelFrame(self, text='配置区',width=800)
    ...
    #     pady = 5
    #     # 选中设备地址文本
    #     self.selectDeviceAddressLable = ttk.Label(self.sendFrame, text="当前设备地址：", font=('微软雅黑', 28))
    ...
    def setFrame_Init(self):
        ...
        serialportLable = ttk.Label(self.set1Frame, width=width, text="串口号:", font=('微软雅黑', 28))
        ...
        BaudrateLable = ttk.Label(self.set2Frame, width=width, text="波特率:", font=('微软雅黑', 28))
        ...
        self.baudrateCombobox = ttk.Combobox(self.set2Frame, width=width, textvariable=self.baudrate, state='readonly',
                                             font=('微软雅黑', 28))
        self.baudrateCombobox['values'] = (1200, 2800, 4800, 9600, 14400, 19200, 38400, 43000, 57600, 76800, 115200)
        ...
        bytesizeLabel = ttk.Label(self.set3Frame, width=width, text="数据位:", font=('微软雅黑', 28))
        ...
        parityLabel = ttk.Label(self.set4Frame, width=width, text="校验位:", font=('微软雅黑', 28))
        ...
        stopbitsLabel = ttk.Label(self.set5Frame, width=width, text="停止位:", font=('微软雅黑', 28))
        ...
        self.set6Frame.pack(side=TOP, fill=BOTH, padx=10)
        # 扫描可用串口按钮
        serialScanButton = tk.ttk.Button(self.set6Frame, text="扫描可用串口", width=20,
                                     command=self.scanSerial,)
        serialScanButton.grid(row=0, column=0, padx=padx*6, pady=pady,sticky=NSEW)

        # 开启关闭串口按钮
        self.count = 0
        self.serialOpenButton = tk.ttk.Button(self.set6Frame, text="打开串口", width=20,
                                          command=self.openSerial)
        self.serialOpenButton.grid(row=0, column=1, padx=padx*25, pady=pady,sticky=NSEW)
        ...
        self.selectDeviceAddressLable = ttk.Label(self.set1Frame, text="当前设备地址：", font=('微软雅黑', 28))
        ...
        deviceList1 = ttk.Label(self.set2Frame,text="设备列表", font=('微软雅黑', 28))
        ...
        self.deviceListCombobox1 = ttk.Combobox(self.set2Frame, width=9, state='readonly',
                                               font=('微软雅黑', 28))
        self.deviceListCombobox1.grid(row=0, column=2, sticky=tk.NSEW, padx=28.50, pady=pady)
        ...
        self.set3Frame.pack(side=TOP, fill=X, padx=10)
        findAddressLabel = ttk.Label(self.set3Frame, text="寻址指令", font=("微软雅黑", 28))
        findAddressLabel.pack(side=LEFT, padx=padx, pady=pady)
        ##！！！ 这里没有判断deviceRespondListBox元素是否为0，得改
        ...
        self.set4Frame.pack(side=TOP, fill=X, padx=10)
        operateRelayLable = ttk.Label(self.set4Frame, text="操作继电器", font=("微软雅黑", 28))
        operateRelayLable.pack(side=LEFT, padx=padx, pady=pady)
...
class detectViewFrame(Frame):
    ...
    ### ----------组件区----------
    def createWidget(self):
        global figureHeight, figureWidth,ticks
        padx = 15
        pady = 15

        # 设置区
        self.frame1 = tk.LabelFrame(self,height=10)
        ...
        self.setFrame.grid(row=0,column=0, padx=5, pady=5,sticky=tk.NSEW)
        # 状态标志
        ...
        detectInformationFrame.pack(side=TOP, anchor=W, fill=X)

        detectInformationLabel = tk.Label(detectInformationFrame, text="检测信息")
        detectInformationLabel.pack(side=TOP, anchor=W, fill=X)
        self.detectInformationScrolledText = scrolledtext.ScrolledText(detectInformationFrame, wrap=tk.WORD, height=11,
                                                                       width=40)
        self.detectInformationScrolledText.pack(side=TOP, fill=X)

        ## 特征曲线图窗口
        self.featureCurveFrame = tk.LabelFrame(self.frame1,width=30)
        # self.featureCurveFrame.grid(row=0, column=2, sticky=tk.NSEW, padx=padx, pady=pady)
        self.featureCurveFrame.grid(row=0,column=1, padx=5, pady=5,sticky=tk.NSEW)
        featureCurveLabel = tk.Label(self.featureCurveFrame, text="实时检测特征曲线")
        featureCurveLabel.pack(side=TOP,fill=BOTH)

        self.fig = plt.figure(figsize=(7,6), dpi=115,facecolor='black',edgecolor='white')  # 创建空画布
        self.ax = plt.subplot(1, 1, 1)
        self.ax.spines['right'].set_color('none')   # 设置右边无边框
        self.ax.spines['top'].set_color('none')     # 设置上边无边框
        self.ax.tick_params(axis='x',
                         color='white',labelcolor='white')  # y轴标签颜色设置
        ...
        plt.xticks(my_x_ticks,size=ticks)
        plt.yticks(my_y_ticks,size=ticks)

        self.ax.spines['top'].set_color('white')  # 设置上‘脊梁’为红色
        self.ax.spines['right'].set_color('white')  # 设置上‘脊梁’为无色
    ...
    def timer_handler(self):
        ...
        :return:
        """
        global figureHeight, figureWidth, timer_time, showPoints, t_timer, y, y_old, time_x,ticks
        t_timer = threading.Timer(timer_time, self.timer_handler)
        t_timer.daemon = True
        t_timer.start()
        ...
        if len(self.new_data) != 0:
            ...
            if time_x % 10 == 0:
                ...
                time_x = 0
                self.ax.patch.set_facecolor("black")
                self.ax.plot([x_bottomlimit, x_toplimit], [threshold, threshold], color="red", linestyle='--',linewidth=3)  # 阈值曲线
                self.ax.tick_params(axis='x',
                                    color='white',labelcolor='white')  # y轴标签颜色设置
                self.ax.tick_params(axis='y',
                                    color='white',labelcolor='white')  # y轴标签颜色设置
                ...
                y = self.new_data[0]
                self.ax.plot([0, 11], [threshold, threshold], color="red", linestyle='--',linewidth=3)
                self.ax.plot([time_x, (time_x + 1)], [y_old, y], color="white",linewidth=3)
                self.canvas.draw()
            else:
                y_old = y
                y = self.new_data[0]
                self.ax.plot([time_x, (time_x + 1)], [y_old, y], color="white",linewidth=3)     # 连接两点的曲线
                self.canvas.draw()
            time_x = time_x + 1
        ...
        while True:
            ...
            while True:
                self.bluetoothStateLabel.config(
                    text="\n\n正在接收来自检测设备的数据……\n" + "设备地址：" + str(address) + "\n")
                newdata = client_sock.recv(30).decode()  # 不断接收数据，每次接收缓冲区 1028 字节
                if newdata == "":
                    break
...
class MyTreeview(tk.ttk.Treeview):
    ...
    def __init__(self, master=None, columnWidth=120, setcolumn=None, **kw):
        ...
        style_value.configure("Treeview", rowheight=60, font=("微软雅黑", 28))
...
if __name__ == "__main__":
    ...
    root.geometry("1560x1080+330+0")
    # root.resizable(0,0) #不可调整大小
    root.iconphoto(False, tk.PhotoImage(file='resource/hacker.png'))

    # 字体设置
    myfont = font.Font(root=root, family='宋体+Times New Roman', size=28,weight=font.NORMAL, slant=font.ROMAN,

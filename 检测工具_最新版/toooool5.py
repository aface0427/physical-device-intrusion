...
# socket通信有问题 是阻塞的，并且自己收不到自己发送的指令
# 解决方案：
# 1.使用非阻塞通信 2.收不到就收不到呗  √ 我看行
import csv
import datetime
import re
import socket
...
import webbrowser
from tkinter import Frame, Tk, LEFT, BOTH, END, BOTTOM, X, Y, HORIZONTAL, RIGHT, TOP, W, CENTER, scrolledtext, DISABLED, \
    NORMAL
...
class Application(Frame):
    ...
    def createMenu(self):
        ...
        menubar = SetMenu(master=self.master,setMenus=menus,items=items,callbacks=callbacks)
...
class controlViewFrame(Frame):
    ...
    def deviceFrame_Init(self):
        ...
        deviceOperateAddEntry = tk.Entry(self.deviceOperateFrame, textvariable=self.deviceOperateAdd, width=10)
        deviceOperateAddEntry.grid(row=0, column=1, sticky=tk.NSEW, padx=padx, pady=pady)

        deviceOperateAddLabel = ttk.Button(self.deviceOperateFrame, text="添加从站", width=10,
                                           command=self.addOperateDevice)
        deviceOperateAddLabel.grid(row=0, column=2, padx=padx, pady=pady)

        deviceOperateDeleteLabel = ttk.Button(self.deviceOperateFrame, text="删除从站", width=10,
                                              command=self.deleteOperateDevice)
        ...
        self.deviceRespondFrame.pack(side=LEFT, fill=BOTH)

        ## 应答设备列表
        deviceRespondLabel = ttk.Label(self.deviceRespondFrame, text="应答设备列表")
        deviceRespondLabel.grid(row=0, column=0, sticky=tk.NSEW, padx=padx, pady=pady)
...
class detectViewFrame(Frame):
    ...
    def createWidget(self):
        ...
        self.treeview = MyTreeview(master=countIntrusionFrame, columnWidth=250,setcolumn=self.columns, columns=self.columns, height=15,
                                   show="headings")  # 表格
        self.treeview.pack(side=TOP, fill=BOTH)
...
class attackViewFrame(Frame):
    ...
    def setFrame_Init(self):
        ...
        pady=5
        ##  IP
        intrusionIPLable = ttk.Label(self.setFrame, width=width, text="入侵设备 IP:")
        ...
        ## 端口号
        intrusionPortLable = ttk.Label(self.setFrame, width=width, text="入侵设备 端口:")
        ...
        # 连接断开按钮
        self.serialOpenButton = tk.ttk.Button(self.setFrame, text="连接服务器", command=lambda:ThreadFunc(self.connectServer))
        self.serialOpenButton.grid(row=2, column=0, padx=padx, pady=pady)
        self.serialCloseButton = tk.ttk.Button(self.setFrame, text="断开连接", command=lambda:ThreadFunc(self.disconnectServer))
        ...
        # 监听按钮
        serialScanButton = tk.Button(self.setFrame, text="开始监听", command=lambda: ThreadFunc(self.listen))
    ...
    def deviceFrame_Init(self):
        ...
        deviceOperateAddLabel = ttk.Button(self.deviceOperateFrame,text="添加从站",width=10,command=self.addOperateDevice)
        deviceOperateAddLabel.grid(row=0, column=2, padx=padx, pady=pady)

        deviceOperateDeleteLabel = ttk.Button(self.deviceOperateFrame, text="删除从站",width=10,command=self.deleteOperateDevice)
        deviceOperateDeleteLabel.grid(row=0, column=3,padx=padx, pady=pady)
...
class MyTreeview(tk.ttk.Treeview):
    ...
    def createWidget(self):
        ...
        items=["保存","-","删除当前数据","清除所有"]
        callbacks=[self.export,"-",self.clear,self.clearAll]

        self.button3Menu =SetMenu(self,items=items,callbacks=callbacks,tearoff=False)

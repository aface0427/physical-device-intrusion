...
from common import ThreadFunc, ExporTreeview, ToolTip, create_ToolTip
...
class Application(Frame):
    ...
    def createMenu(self):
        ...
        items = [['导出……','导出所有……'],
                 ['选择语言'],
                 ['使用说明']]
        callbacks = [[self.export, self.exportAll],
                     [self.changeLanguage],
                     [self.help]]
    ...
    def createToolbar(self):
        ...
        imgSize = 20
        toolName = ['save', 'saveAll','delete']  # 图片文件名
    ...
    def export(self):
        ...
        # 打开选择文件夹
        excel_name = asksaveasfilename(initialfile=sheet_name, defaultextension='.xlsx',
...
class controlViewFrame(Frame):
    ...
    def dataFrame_Init(self):
        ...
        scrollBar = tk.Scrollbar(self.dataFrame)
        scrollBarx = tk.Scrollbar(self.dataFrame, orient=HORIZONTAL)

        #  每行15个数据
        self.treeview = ttk.Treeview(self.dataFrame, height=15, show="headings", columns=columns,
                                     yscrollcommand=scrollBar.set, xscrollcommand=scrollBarx.set)  # 表格

        for i in columns:
            self.treeview.column(i, width=150, anchor='center')  # 表示列,不显示
            self.treeview.heading(i, text=i)  # 显示表头
        ...
        # 测试数据
        # while (self.treeviewNum < 100):
        #     p = [self.treeviewNum, '1999', '03', '01', '00010001']
    ...
    def sendReadDeviceAddressCommand(self, num: str):
        try:
            ...
            self.treeview.insert('', index=END,
                                 values=(self.treeviewNum, datetime.datetime.now()) + splitCommand(str(data1.hex())) + (
                                 "发送",))
            self.treeviewNum += 1
            ...
            if len_return_data:
                ...
                # 更新excel表
                self.treeview.insert('', index=END,
                                     values=(self.treeviewNum, datetime.datetime.now()) + splitCommand(
                                         str_return_data) + ("接收",))
                self.treeviewNum += 1
        except Exception as e:
            res = messagebox.showerror(title='Error', message=e)
...
class detectViewFrame(Frame):
    ...
    def __init__(self, master=None, cnf={}, **kw):
        ...
        self.historyFeatureList = tk.StringVar()  # 历史数据
        self.historyFeatureIndex = 0  # 历史入侵序号
    ...
    def createWidget(self):
        ...
        # 添加滚动条
        # 滚动条初始化（scrollBar为垂直滚动条，scrollBarx为水平滚动条）
        scrollBar = tk.Scrollbar(countIntrusionFrame)
        scrollBarx = tk.Scrollbar(countIntrusionFrame, orient=HORIZONTAL)

        #  每行15个数据
        self.treeview = ttk.Treeview(countIntrusionFrame, height=15, show="headings", columns=self.columns,
                                     yscrollcommand=scrollBar.set, xscrollcommand=scrollBarx.set)  # 表格

        for i in self.columns:
            self.treeview.column(i, anchor='center')  # 表示列,不显示
            self.treeview.heading(i, text=i)  # 显示表头
        ...
        # 测试数据
        # while (self.IntrusionTreeviewNum < 100):
        #     p = [self.IntrusionTreeviewNum, '1999', '03', '01', '00010001']
        #     self.treeview.insert('', index=END, values=tuple(p))
        #     self.IntrusionTreeviewNum += 1
        ...
        if len(self.new_data) != 0:
            ...
            if self.new_data[0] > threshold:
                ...
                if self.intrusionFlag == 0:
                    ...
                    self.treeview.insert('', index=END, iid=self.historyFeatureIndex, values=(
                    self.historyFeatureIndex, time_str, '',round(self.new_data[0],3), 0))
                else:
                    self.intrusionFlag = 1
                    self.treeview.set(self.historyFeatureIndex,column=self.columns[-1],value=(now_time - self.intrusionBeginTime).seconds)
                ...
                if self.intrusionFlag==1:
                    #设置入侵结束时间
                    self.treeview.set(self.historyFeatureIndex,column=self.columns[2],value=time_str)
                    self.historyFeatureIndex += 1
                # 标志位清零
...
class attackViewFrame(Frame):
    ...
    def createWidget(self):
        ...
        self.button3Menu.add_command(label="删除当前数据", command=self.clearOne)
    ...
    def onTreeviewSelect(self,event):
        ...
        :return:
        """
        self.selectTreeviewItems = self.treeview.selection()
    ...
    def listen(self):
        try:
            ...
            while True:
                ...
                # 更新excel表
                self.treeview.insert('', index=END,
                                     values=(self.treeviewNum, datetime.datetime.now()) + splitCommand(data))
                self.treeviewNum+=1
        except Exception as e:
            res = messagebox.showerror(title='Error', message=e)

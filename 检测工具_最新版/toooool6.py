...
from common import ThreadFunc, ExporTreeview, create_ToolTip, SetMenu, findAllChildrens, findTypeChildrens, \
    bindButtonDisable, translator, MyTreeview
    ...
    def setFont(self):
        ...
        for child in childrens:
            ...
            if ('style' in child.config()) and (child.widgetName.split('::')[0]=='ttk'):
                ...
                sb = ttk.Style()
                if typea[0]=='T':
                    sb.configure('m.' + typea, font=myfont)
                    child.config(style='m.' + typea)
                else:
                    sb.configure('m.T'+typea, font=myfont)
                    child.config(style='m.T' + typea)
...
class controlViewFrame(Frame):
    ...
    def deviceFrame_Init(self):
        ...
        deviceOperateAddLabel = ttk.Button(self.deviceOperateFrame, text="添加从站",
                                           command=self.addOperateDevice)
        deviceOperateAddLabel.grid(row=0, column=2, padx=padx, pady=pady)

        deviceOperateDeleteLabel = ttk.Button(self.deviceOperateFrame, text="删除从站",
...
class attackViewFrame(Frame):
    ...
    def deviceFrame_Init(self):
        ...
        deviceOperateAddLabel = ttk.Button(self.deviceOperateFrame,text="添加从站",command=self.addOperateDevice)
        deviceOperateAddLabel.grid(row=0, column=2, padx=padx, pady=pady)

        deviceOperateDeleteLabel = ttk.Button(self.deviceOperateFrame, text="删除从站",command=self.deleteOperateDevice)
        deviceOperateDeleteLabel.grid(row=0, column=3,padx=padx, pady=pady)
...
if __name__ == "__main__":
    ...
    myfont = font.Font(root=root, family='宋体', size=15, weight=font.NORMAL, slant=font.ROMAN, underline=0, overstrike=0)

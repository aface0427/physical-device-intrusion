...
gatetcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # 网关 TCP 连接对象
tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # TCP 连接对象
timer_time = 5  # 曲线图显示数据时间间隔
timeout_in_seconds = 0.1    # 阻塞等待时间
...
y_toplimit = 50  # 实时检测特征曲线y轴上限限
x_scale = 1  # 实时检测特征曲线x轴刻度
y_scale = 5  # 实时检测特征曲线y轴刻度

# 系统初始化
t_timer = None  # 定时器对象
plt_threhold = None  # 阈值曲线对象
y_old = 0
y = 0
time_x = 0
plt_threshold_old_value=0
plt_threshold_value=0
...
class Application(Frame):
    ...
    def createToolbar(self):
        ...
        for i in range(len(toolName)):
            ...
            toolButton.append(
                ttk.Button(toolframe, bootstyle='link', width=20, image=self.tool[i], command=toolFunc[i]))
...
class controlViewFrame(Frame):
    ...
    def setFrame_Init(self):
        ...
        selectDeviceOperation = ['访问控制区', '操作继电器']  # 选项卡名称
        ...
        intrusionIPLable = ttk.Label(self.set1Frame, width=15, text="网关 IP:", font=('微软雅黑', 12))
        intrusionIPLable.grid(row=0, column=0, sticky=tk.NSEW, padx=padx, pady=pady)  # 添加串口号标签
        self.intrusionIP = tk.StringVar(value="10.18.18.94")  # 端口号字符串
        ...
        ## 端口号
        intrusionPortLable = ttk.Label(self.set1Frame, width=15, text="网关 端口:", font=('微软雅黑', 12))
        ...
        self.set2Frame.pack(side=TOP, fill=X, padx=10, pady=30, ipadx=10)
        self.serialOpenButton = tk.ttk.Button(self.set2Frame, text="连接网关",
                                              command=lambda: ThreadFunc(self.connectServer), width=10)
        self.serialOpenButton.grid(row=0, column=0, padx=20, pady=pady)
        self.serialCloseButton = tk.ttk.Button(self.set2Frame, text="断开连接",
                                               command=lambda: ThreadFunc(self.disconnectServer), width=10)
        self.serialCloseButton.grid(row=0, column=1, padx=10, pady=pady)
        ...
        findAddressButton = tk.ttk.Button(self.set3Frame, text="发送",
                                          command=lambda: self.sendReadDeviceAddressCommand(
                                              self.deviceOperateAddEntry1.get()))
        findAddressButton.pack(side=LEFT, padx=21.50, pady=pady)

        # 开关继电器指令
        self.set1Frame = tk.Frame(self.operate3Frame)
        self.set1Frame.pack(side=TOP, fill=X, padx=10)
        operateRelayLable = ttk.Label(self.set1Frame, text="操作继电器", font=("微软雅黑", 13))
        operateRelayLable.pack(side=LEFT, padx=padx, pady=pady)
        connectRelayButton = tk.ttk.Button(self.set1Frame, text='吸合',
                                           command=lambda: self.connectRelay(self.deviceOperateAddEntry1.get()))
        connectRelayButton.pack(side=LEFT, padx=padx, pady=pady)
        disconnectRelayButton = tk.ttk.Button(self.set1Frame, text='断开',
        ...
        self.sendFrame_requestOrder = tk.Frame(self.set4Frame)
        ...
        sendFrame_returnOrder = tk.Frame(self.set5Frame)
    ...
    def connectServer(self):
        ...
        :return:
        """
        global gatetcp_socket
        try:
            gatetcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # TCP 连接对象
            # 修改界面状态
            self.TCPSocketStateLabel['text'] = "\n\n正在连接到网关设备，请稍候……\n\n"
            server_addr = (self.intrusionIP.get(), self.intrusionPort.get())
            gatetcp_socket.connect(server_addr)
            ...
            # # 监听端口
            # while True:
            #     self.data = gatetcp_socket.recv(self.buff_size)

            #     if self.needselfRecvFlag == 1:  # 寻找地址时接收指令
    ...
    def disconnectServer(self):
        ...
        try:
            if (gatetcp_socket):
                gatetcp_socket.shutdown(2)
                gatetcp_socket.close()
                self.TCPSocketStateLabel['text'] = "\n\n网关设备已断开连接\n\n"
    ...
    def sendReadDeviceAddressCommand(self, num: str):
        try:
            ...
            if self.data:
                # bytes(2进制)转换为hex(16进制)，应注意Python3.7与Python2.7此处转换的不同，并转为字符串后截取所需数据字段，再转为10进制
                str_return_data = str(self.data.hex())
        ...
        try:
            ...
            while (cycle < cycle_max):
                ...
                for i in range(1, 16):  # 1-255地址轮询
                    ...
                    self.treeview.yview_moveto(1.0)
                    self.treeview.treeviewNum += 1
                    gatetcp_socket.send(data1)  # TCP 发送数据
                    time1 = time.perf_counter()
                    time.sleep(0.1)
                    while True:
                        if ((time.perf_counter() - time1) * 1000) > 50:
                    ...
                    # 设置阻塞等待，直到数据可用或超时时执行
                    gatetcp_socket.setblocking(0)
                    ready = select.select([gatetcp_socket], [], [], timeout_in_seconds)
                    if ready[0]:
                        self.data = gatetcp_socket.recv(self.buff_size)
                        if self.data:
                            # bytes(2进制)转换为hex(16进制)，应注意Python3.7与Python2.7此处转换的不同，并转为字符串后截取所需数据字段，再转为10进制
    ...
    def connectRelay(self, num):
        ...
        self.requestCommand.config(text=str(send_data.hex()))
        gatetcp_socket.send(send_data)  # TCP 发送数据
...
class detectViewFrame(Frame):
    ...
    def timer_handler(self):
        ...
        # print("timer",threading.active_count())
        # print(threading.enumerate())

        global figureHeight, figureWidth, timer_time, showPoints, t_timer, y, y_old, time_x, plt_threhold, plt_threshold_old_value,plt_threshold_value
        ...
        if len(self.new_data) != 0:
            ...
            if time_x % 10 == 0:
                ...
                plt.ylabel("特征值/10log10")
                # plt.xlabel("特征序列")
                my_x_ticks = np.arange(x_bottomlimit, x_toplimit, x_scale)  # 设置横坐标刻度
                my_y_ticks = np.arange(y_bottomlimit, y_toplimit, y_scale)  # 设置纵坐标刻度
                plt.xticks(my_x_ticks)
                plt.yticks(my_y_ticks)
                y_old = self.log_scale(self.new_data[0])  # 10log10
                y = self.log_scale(self.new_data[0])  # 10log10
                #更新阈值对象
                plt_threshold_old_value=self.log_scale(self.new_data[1]) # 10log10
                plt_threshold_value = self.log_scale(self.new_data[1])# 10log10
                self.ax.plot([time_x, (time_x + 1)], [plt_threshold_old_value, plt_threshold_value], color="red", linestyle='--')
                ...
                y = self.log_scale(self.new_data[0])  # 10log10
                # 更新阈值对象
                plt_threshold_old_value = plt_threshold_value # 10log10
                plt_threshold_value = self.log_scale(self.new_data[1]) # 10log10
                self.ax.plot([time_x, (time_x + 1)], [plt_threshold_old_value, plt_threshold_value], color="red", linestyle='--')
    ...
    def log_scale(self,y):
        """
        对y进行10log10缩放
        """
        return 10*math.log(y+1,10)
    ...
    def thread_server_listen(self):
        ...
        # print("listen", threading.active_count())
        # print(threading.enumerate())
...
class attackViewFrame(Frame):
    ...
    def sendFrame_Init(self):
        ...
        connectRelayButton = tk.Button(sendFrame_operateRelay, text='吸合',
                                       command=lambda: self.connectRelay(self.selectDeviceAddress), height=1, width=6,
                                       font=('微软雅黑', 15))
        connectRelayButton.pack(side=LEFT, padx=padx, pady=pady)
        disconnectRelayButton = tk.Button(sendFrame_operateRelay, text='断开',
                                          command=lambda: self.disconnectRelay(self.selectDeviceAddress), height=1,
                                          width=6, font=('微软雅黑', 15))
        disconnectRelayButton.pack(side=LEFT, padx=padx, pady=pady)
    ...
    def setFrame_Init(self):
        ...
        selectDeviceOperationNotebook = tk.ttk.Notebook(self.setFrame)
        selectDeviceOperationNotebook.grid(row=0, column=1, sticky=tk.NSEW, padx=padx, pady=pady)
        selectDeviceOperation = ['访问控制区', '应答伪造区', '操作继电器']  # 选项卡名称
        self.operate2Frame = Frame(self.setFrame)
        self.operate2Frame.grid(row=0, column=1, sticky=tk.NSEW, padx=padx, pady=pady)
        selectDeviceOperationNotebook.add(self.operate2Frame, text=selectDeviceOperation[0])
        self.operate3Frame = Frame(self.setFrame)
        self.operate3Frame.grid(row=0, column=1, sticky=tk.NSEW, padx=padx, pady=pady)
        selectDeviceOperationNotebook.add(self.operate3Frame, text=selectDeviceOperation[1])
        self.operate4Frame = Frame(self.setFrame)
        self.operate4Frame.grid(row=0, column=1, sticky=tk.NSEW, padx=padx, pady=pady)
        selectDeviceOperationNotebook.add(self.operate4Frame, text=selectDeviceOperation[2])
        ...
        intrusionIPLable.grid(row=0, column=0, sticky=tk.NSEW, padx=padx, pady=pady)  # 添加串口号标签
        self.intrusionIP = tk.StringVar(value="10.18.18.93")  # 端口号字符串
        self.intrusionIPCombobox = ttk.Entry(self.set1Frame, width=13, textvariable=self.intrusionIP, font=15)
        self.intrusionIPCombobox.grid(row=0, column=1, padx=padx, pady=pady)  # 设置其在界面中出现的位置  column代表列   row 代表行
        ...
        # 连接断开按钮
        self.set2Frame = tk.Frame(self.operate1Frame)
        self.set2Frame.pack(side=TOP, fill=X, padx=10, pady=30)
        self.serialOpenButton = tk.ttk.Button(self.set2Frame, text="连接服务器",
                                              command=lambda: ThreadFunc(self.connectServer), width=10)
        ...
        # 监听按钮
        serialScanButton = tk.ttk.Button(self.set2Frame, text="开始监听", command=lambda: ThreadFunc(self.listen),
                                         width=10)
        ...
        ## 设备列表
        blank = ttk.Label(self.set2Frame, text="")
        blank.grid(row=0, column=2, sticky=tk.NSEW, padx=11.5, pady=pady)
        self.deviceList = tk.StringVar()  # 绑定字符串
        self.deviceListCombobox1 = ttk.Combobox(self.set2Frame, width=4, state='readonly',
                                                font=('微软雅黑', 12))
        self.deviceListCombobox1.grid(row=0, column=3, sticky=tk.NSEW, padx=24.5, pady=pady)
        self.deviceListCombobox1.bind('<<ComboboxSelected>>', self.callback)

        finddeviceButton = ttk.Button(self.set2Frame, text="寻找设备",
                                      command=lambda: ThreadFunc(self.findDevice))
        finddeviceButton.grid(row=0, column=4, sticky=tk.NSEW, padx=10, pady=pady)

        self.cycleFindDeviceButton = ttk.Checkbutton(self.set2Frame, text="循环发送",
                                                     variable=self.cycleFindDeviceFlag)
        self.cycleFindDeviceButton.grid(row=0, column=5, sticky=tk.NSEW, padx=10, pady=pady)
        ...
        self.set4Frame = tk.Frame(self.operate2Frame)
        self.set4Frame.pack(side=TOP, fill=X, padx=10)
        dosLable = ttk.Label(self.set4Frame, text="拒绝服务攻击", font=("微软雅黑", 13))
        dosLable.pack(side=LEFT, padx=padx, pady=pady)
        openButton = tk.ttk.Button(self.set4Frame, text='开启',
                                           command=lambda: self.connectRelay(self.deviceOperateAddEntry1.get()))
        openButton.pack(side=LEFT, padx=padx, pady=pady)
...
if __name__ == "__main__":
    ...
    # chazhao=threading.Thread(target=chaya)
    # chazhao.daemon=True
    # chazhao.start()

...
class controlViewFrame(Frame):
    ...
    def deviceFrame_Init(self):
        ...
        pady = 5
        ## 设备列表
        deviceRespondLabel = ttk.Label(self.deviceFrame, text="设备列表")
        deviceRespondLabel.grid(row=0, column=0, sticky=tk.NSEW, padx=padx, pady=pady)

        # 寻找设备按钮
        finddeviceButton = ttk.Button(self.deviceFrame, text="寻找设备", command=lambda :ThreadFunc(self.findDevice))
        ...
        # 创建Listbox
        self.deviceRespondListBox = tk.Listbox(self.deviceFrame, height=16, listvariable=self.deviceRespondList, selectmode='browse')
        self.deviceRespondListBox.grid(row=1, column=0, columnspan=3, sticky=tk.NSEW, padx=padx, pady=pady)
        self.deviceRespondListBox.bind('<Double-Button-1>', self.changeSelectDevice)
    ...
    def changeSelectDevice(self, event):
        ...
        """
        try:
            activeDeviceIndex = self.deviceRespondListBox.curselection()[0]
            self.selectDeviceAddress = self.deviceRespondListBox.get(activeDeviceIndex, activeDeviceIndex)[0]
...
class detectViewFrame(Frame):
    ...
    def createWidget(self):
        ...
        countIntrusionFrame.grid(row=1, column=0, columnspan=3, sticky=tk.NSEW, padx=padx, pady=pady)
...
class attackViewFrame(Frame):
    ...
    def __init__(self, master=None, cnf={}, **kw):
        ...
        self.deviceOperateList= tk.StringVar() #操纵从机设备列表
...
class MyTreeview(tk.ttk.Treeview):
    ...
    def createWidget(self):
        ...
        for i in self.setcolumn:
            # print(i)
            self.column(i, anchor='center',width=150)  # 表示列,不显示

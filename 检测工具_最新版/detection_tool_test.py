...
class controlViewFrame(Frame):
    ...
    def createWidget(self):
        ...
        self.setFrame_Init()
        # 数据显示区
        self.dataFrame = tk.ttk.LabelFrame(self,text='数据收发区',width=100)
    ...
    def sendFrame_Init(self):
        ...
        self.sendFrame_request = tk.LabelFrame(self.sendFrame_requestOrder, text="请求栏", width=700, height=60)
        ...
        self.requestCommand = tk.Label(self.sendFrame_requestFrame, text="此处为请求指令信息")
        ...
        sendFrame_return = tk.LabelFrame(sendFrame_returnOrder, text="应答栏", width=700, height=60)
        ...
        self.returnCommand = tk.Label(sendFrame_returnFrame, text="此处为应答指令信息")
        ...
        operateAddressButton = tk.Button(sendFrame_operateAddress, text="发送请求指令",
                                      command=lambda: self.sendOperateDeviceAddressCommand(self.selectDeviceAddress))      # 单独写一个函数
        operateAddressButton.pack(side=LEFT, padx=padx, pady=pady)
        ...
        sendFrame_operaterequest = ttk.Label(sendFrame_operaterequestOrder, text="待发送指令")
        sendFrame_operaterequest.grid(row=0, column=0, sticky=tk.NSEW, padx=padx, pady=pady)
        self.OperateAddEntry = tk.Entry(sendFrame_operaterequestOrder, width=70)
        ...
        sendFrame_operatereturn = tk.LabelFrame(sendFrame_operatereturnOrder, text="应答栏", width=700, height=60)
        ...
        self.operatereturnCommand = tk.Label(sendFrame_operatereturnFrame, text="此处为应答指令信息")
    ...
    def sendOperateDeviceAddressCommand(self, num: str):               # 新增操作寄存器按钮绑定事件函数
        try:
            ...
            self.treeview.insert('', index=END,
                                 values=(self.treeview.treeviewNum, datetime.datetime.now()) + splitCommand(str(findaddress_str.hex())) + (
                                 "发送",))
            self.treeview.treeviewNum += 1
            time1 = time.perf_counter()
            while True:
...
class detectViewFrame(Frame):
    ...
    def __init__(self, master=None, cnf={}, **kw):
        ...
        t_bluetooth_listen.setDaemon(True)
    ...
    def timer_handler(self):
        ...
        global figureHeight, figureWidth, timer_time, showPoints,t_timer
        t_timer = threading.Timer(timer_time, self.timer_handler)
        t_timer.setDaemon(True)
        t_timer.start()
...
class attackViewFrame(Frame):
    ...
    def deviceFrame_Init(self):
        ...
        sendFrame_findAddress = ttk.Label(sendFrame_answerForgeOrder1, text="待发送指令")
        ...
        self.findAddressAddEntry = tk.Entry(sendFrame_answerForgeOrder2, width=30)
        ...
        self.sendFrame_answerForge = ttk.Label(sendFrame_answerForgeOrder3, text="应答伪造指令")
        ...
        self.answerOrderAddEntry = tk.Entry(sendFrame_answerForgeOrder4, width=30)
        ...
        self.treeview1 = MyTreeview(master=self.answerForgeFrame2,setcolumn=columns,columns=columns,height=13, show="headings")  # 表格
    ...
    def orderBindOperation(self):
        ...
        try:
            ...
            findaddress_str2 = findaddress_str2 + findaddress_data2
             ...
             # 更新excel表
            self.treeview1.insert('', index=END, values=(self.treeview1.treeviewNum, datetime.datetime.now()) + (str(findaddress_str1.hex()),) + (str(findaddress_str2.hex()),))
            time1 = time.perf_counter()
            while True:
                if ((time.perf_counter() - time1) * 1000) > 50:
                    break
